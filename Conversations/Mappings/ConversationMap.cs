namespace Conversations.Mappings
{
	...
	internal class ConversationMap<TAuthorKey, TComment> : IEntityTypeConfiguration<Conversation<TAuthorKey, TComment>>
		where TComment : Comment<TAuthorKey>, new()
	{
		...
		public void Configure(EntityTypeBuilder<Conversation<TAuthorKey, TComment>> entity)
		{
			...
			entity.Property(t => t.Key).HasColumnName("Key").HasMaxLength(Conversation<TAuthorKey, TComment>.KeyMaxLength).IsUnicode(false).IsRequired();
